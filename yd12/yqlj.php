...
if($_POST[action]=="add"){
$Result=$conn->query("INSERT INTO `yk_yqlj` (`id`, `name`, `url`, `QQ`) VALUES (NULL, '$_POST[name]', '$_POST[url]', '$_POST[QQ]')");
 if($Result==1){
  echo '<script>alert("添加成功！");window.location.href="yqlj.php"</script>';
}

}
if($_GET[action]=="del"){
  $Result=$conn->query("DELETE FROM `yk_yqlj` WHERE `id`='$_GET[id]'");
  if ($Result==1) {
    echo '<script>alert("已删除");window.location.href="yqlj.php"</script>';
...
<html lang="zh-CN">
  ...
  <body>
   ...
   <form action="yqlj.php" method="post">
   ...
   友情链接<!-- <button class="btn btn-xs btn-success" style="float:right"><i class="glyphicon glyphicon-plus" onclick="document.getElementById("form").style.display="inline-block"">添加</i></button> -->
   </div>
   <div class="row">
...
名称
   ...
   while($R_yqlj=$A_yqlj->fetch_array()){
    ...
    <td>
    <a href="yqlj.php?action=del&id=<?php echo $R_yqlj[id] ?>">删除</a>
    </td>
